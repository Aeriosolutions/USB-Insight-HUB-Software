 ...
 203  0020 84            	pop	a
 204  0021 81            	ret
 243                     ; 86 	u8 I2C_byte_write(void)
 243                     ; 87 	{
 244                     	switch	.text
 245  0022               _I2C_byte_write:
 249                     ; 90 		if(reg_address == WHOAMI) 				return WHOAMI_ID;
 ...
 259  002a 81            	ret
 260  002b               L36:
 261                     ; 91 		else if(reg_address == VERSION) 	return VERNUM;
 ...
 271  0033 81            	ret
 272  0034               L76:
 273                     ; 92 		else if(reg_address == CH1REG) 		return r20_CH1REG;
 ...
 283  003c 81            	ret
 284  003d               L37:
 285                     ; 93 		else if(reg_address == CH2REG) 		return r21_CH2REG;
 ...
 295  0045 81            	ret
 296  0046               L77:
 297                     ; 94 		else if(reg_address == CH3REG) 		return r22_CH3REG;
 ...
 307  004e 81            	ret
 308  004f               L301:
 309                     ; 95 		else if(reg_address == CCSUM) 		return r23_CCSUM;
 ...
 319  0057 81            	ret
 320  0058               L701:
 321                     ; 96 		else if(reg_address == AUXREG){
 323  0058 b603          	ld	a,_reg_address
 324  005a a124          	cp	a,#36
 325  005c 2605          	jrne	L311
 326                     ; 97 			firstPowerFlag = FALSE;
 328  005e 3f01          	clr	_firstPowerFlag
 329                     ; 98 			return r24_AUXREG;
 331  0060 b608          	ld	a,_r24_AUXREG
 334  0062 81            	ret
 335  0063               L311:
 336                     ; 100 		else if(reg_address == MUXOECTR) 	return r26_MUXOECTR;
 ...
 346  006b 81            	ret
 347  006c               L711:
 348                     ; 101 		else if(reg_address == VEXTCC1L) 	return r30_VEXTCC1L;
 ...
 358  0074 81            	ret
 359  0075               L321:
 360                     ; 102 		else if(reg_address == VEXTCC1H) 	return r31_VEXTCC1H;
 ...
 370  007d 81            	ret
 371  007e               L721:
 372                     ; 103 		else if(reg_address == VEXTCC2L) 	return r32_VEXTCC2L;
 ...
 382  0086 81            	ret
 383  0087               L331:
 384                     ; 104 		else if(reg_address == VEXTCC2H) 	return r33_VEXTCC2H;
 ...
 394  008f 81            	ret
 395  0090               L731:
 396                     ; 105 		else if(reg_address == VHOSTCC1L) return r34_VHOSTCC1L;
 ...
 406  0098 81            	ret
 407  0099               L341:
 408                     ; 106 		else if(reg_address == VHOSTCC1H) return r35_VHOSTCC1H;
 ...
 418  00a1 81            	ret
 419  00a2               L741:
 420                     ; 107 		else if(reg_address == VHOSTCC2L) return r36_VHOSTCC2L;
 ...
 430  00aa 81            	ret
 431  00ab               L351:
 432                     ; 108 		else if(reg_address == VHOSTCC2H) return r37_VHOSTCC2H;		
 ...
 442  00b3 81            	ret
 443  00b4               L751:
 444                     ; 109 		else return 0x00;
 446  00b4 4f            	clr	a
 449  00b5 81            	ret
 497                     ; 112 	void Writable_Registers(u8 reg_address, u8 u8_RxData)
 497                     ; 113 	{
 498                     	switch	.text
 499  00b6               _Writable_Registers:
 501  00b6 89            	pushw	x
 502       00000000      OFST:	set	0
 505                     ; 114 		if(reg_address==CH1REG){
 507  00b7 9e            	ld	a,xh
 508  00b8 a120          	cp	a,#32
 509  00ba 260a          	jrne	L502
 510                     ; 116 			r20_CH1REG = (r20_CH1REG | 0x7F ) & u8_RxData; 
 ...
 518  00c6               L502:
 519                     ; 118 		else if(reg_address ==CH2REG){
 521  00c6 7b01          	ld	a,(OFST+1,sp)
 522  00c8 a121          	cp	a,#33
 523  00ca 260a          	jrne	L112
 524                     ; 119 			r21_CH2REG = (r21_CH2REG | 0x7F ) & u8_RxData;
 ...
 532  00d6               L112:
 533                     ; 121 		else if(reg_address ==CH3REG){
 535  00d6 7b01          	ld	a,(OFST+1,sp)
 536  00d8 a122          	cp	a,#34
 537  00da 260a          	jrne	L512
 538                     ; 122 			r22_CH3REG = (r22_CH3REG | 0x7F ) & u8_RxData;
 ...
 573  0002               L722_sr3:
 574  0002 00            	ds.b	1
 630                     ; 136 @far @interrupt void I2C_Slave_check_event(void) {
 632                     	switch	.text
 ...
 769  0192 80            	iret
 791                     ; 199 void Init_I2C (void)
 791                     ; 200 {
 793                     	switch	.text
 794  0193               _Init_I2C:
 798                     ; 203 	CLK->CKDIVR = 0;                
 800  0193 725f50c6      	clr	20678
 801                     ; 205 	GPIOB->CR1 |= 0x30;
 803  0197 c65008        	ld	a,20488
 804  019a aa30          	or	a,#48
 805  019c c75008        	ld	20488,a
 806                     ; 206 	GPIOB->DDR &= ~0x30;
 808  019f c65007        	ld	a,20487
 809  01a2 a4cf          	and	a,#207
 810  01a4 c75007        	ld	20487,a
 811                     ; 207 	GPIOB->CR2 &= ~0x30;	
 813  01a7 c65009        	ld	a,20489
 814  01aa a4cf          	and	a,#207
 815  01ac c75009        	ld	20489,a
 816                     ; 210 	I2C->CR1 |= 0x01;				        	// Enable I2C peripheral
 818  01af 72105210      	bset	21008,#0
 819                     ; 211 	I2C->CR2 = 0x04;					      		// Enable I2C acknowledgement
 821  01b3 35045211      	mov	21009,#4
 822                     ; 212 	I2C->FREQR = 16; 					      	// Set I2C Freq value (16MHz)
 824  01b7 35105212      	mov	21010,#16
 825                     ; 213 	I2C->OARL = (SLAVE_ADDRESS << 1) ;	// set slave address to 0x51 (put 0xA2 for the register dues to7bit address) 
 827  01bb 35a25213      	mov	21011,#162
 828                     ; 214 	I2C->OARH = 0x40;					      	// Set 7bit address mode
 830  01bf 35405214      	mov	21012,#64
 831                     ; 216 	I2C->ITR	= 0x07;					      // all I2C interrupt enable  
